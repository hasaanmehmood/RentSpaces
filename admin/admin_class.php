...
Class Action {
	...
	function save_category(){
		...
		$data .= ", uid = '$uid' ";
		if(empty($id)){
			$save = $this->db->query("INSERT INTO spaces set ".$data);
		}else{
			$save = $this->db->query("UPDATE spaces set ".$data." where id=".$id);
		}
	...
	function delete_category(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM spaces where id = ".$id);
		if($delete)
			return 1;
	}
	function save_room(){
